...
impl Locator {
    ...
    pub async fn all(&self) -> SkyVMResult<Vec<Element>> {
        let mut inner = self.inner.lock().await;
        inner.clear_cache();
        let tree = inner
            .get_tree(&TreeFilter::default())
            .await
            .map_err(|e| SkyVMError::Other(e))?;

        let selector = parse_query(&self.selector).map_err(|e| SkyVMError::InvalidSelector {
    ...
    pub async fn get(&self) -> SkyVMResult<Option<Element>> {
    ...
    pub async fn wait(&self) -> SkyVMResult<Element> {
        self.poll_for_element().await
    }

    /// Wait until the element is visible (has bounds).
    pub async fn wait_until_visible(&self) -> SkyVMResult<Element> {
        self.poll_for_condition(|elem| elem.bounds.is_some(), "element visible")
            .await
    }

    /// Wait until the element is enabled.
    pub async fn wait_until_enabled(&self) -> SkyVMResult<Element> {
    ...
    ///     .await?;
    /// ```
    pub async fn wait_for_value(&self, pattern: &str) -> SkyVMResult<Element> {
    ...
    pub async fn wait_for_text(&self, pattern: &str) -> SkyVMResult<Element> {
        let pattern = pattern.to_string();
        self.poll_for_condition(
            |elem| {
                let in_title = elem
    ...
    ///     .await?;
    /// ```
    pub async fn wait_for<F>(&self, predicate: F, description: &str) -> SkyVMResult<Element>
    where
        F: Fn(&Element) -> bool,
    {
    ...
    /// Focus the element.
    pub async fn focus(&self) -> SkyVMResult<Element> {
        let elem = self.poll_for_element().await?;
    ...
    pub async fn blur(&self) -> SkyVMResult<Element> {
        let elem = self.poll_for_element().await?;
    ...
    pub async fn fill(&self, text: &str) -> SkyVMResult<Element> {
        let elem = self.poll_for_element().await?;
    ...
    pub async fn type_text(&self, text: &str) -> SkyVMResult<Element> {
        let elem = self.poll_for_element().await?;
    ...
    pub async fn keystroke(&self, key_spec: &str) -> SkyVMResult<Element> {
        let elem = self.poll_for_element().await?;
    ...
    pub async fn value(&self) -> SkyVMResult<Option<String>> {
        let elem = self.poll_for_element().await?;
        Ok(elem.value.clone())
    }

    /// Get the element's title.
    pub async fn title(&self) -> SkyVMResult<Option<String>> {
        let elem = self.poll_for_element().await?;
        Ok(elem.title.clone())
    }
    ...
    async fn find_element_now(&self) -> SkyVMResult<Option<Element>> {
        let mut inner = self.inner.lock().await;
        inner.clear_cache();
        let tree = inner
            .get_tree(&TreeFilter::default())
        ...
        if matches.len() > 1 && self.options.strict {
            return Err(SkyVMError::MultipleMatches {
                selector: self.selector.clone(),
                count: matches.len(),
            });
        }
    ...
    async fn poll_for_element(&self) -> SkyVMResult<Element> {
        ...
        loop {
            match self.find_element_now().await {
                ...
                Ok(None) => {}
                Err(e) if !matches!(e, SkyVMError::ElementNotFound { .. }) => {
                    return Err(e);
                }
                Err(_) => {}
            }
            ...
            if timeout == Duration::ZERO || start.elapsed() >= timeout {
                return Err(SkyVMError::ElementNotFound {
                    selector: self.selector.clone(),
                });
            }
        ...
        F: Fn(&Element) -> bool,
    {
        ...
        loop {
            ...
            match self.all().await {
                ...
                Err(e) if !matches!(e, SkyVMError::ElementNotFound { .. }) => return Err(e),
                Err(_) => {} // No elements found, keep waiting
            }

            if timeout == Duration::ZERO || start.elapsed() >= timeout {
                return Err(SkyVMError::ConditionNotMet {
                    selector: self.selector.clone(),
