...
fn screenshot_path() -> std::path::PathBuf {
    ...
    let timestamp = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let random_id = std::process::id() ^ (timestamp as u32);
    std::env::temp_dir().join(format!("{:x}.png", random_id))
}

// ============================================================================
// Unified Operation Handlers (work with any AccessibilityReader)
// ============================================================================
            ...
            if args.overlay {
                match adapter.get_screen_bounds().await {
                    ...
                    Err(e) => {
                        ...
                        let filename = screenshot_path();
                        std::fs::write(&filename, &screenshot.data)
                ...
                let filename = screenshot_path();
                std::fs::write(&filename, &screenshot.data).expect("Failed to write file");
                println!(
...
async fn handle_annotate(
    ...
    let filename = screenshot_path();
    img.save(&filename).expect("Failed to save annotated image");
    println!(
    ...
    for (i, elem) in elements.iter().enumerate() {
        if let Some(bounds) = &elem.bounds {
            match screenshot.crop(bounds, &screen_bounds) {
                Ok(cropped) => {
                    let filename = screenshot_path();
                    std::fs::write(&filename, &cropped.data).expect("Failed to write file");
                    println!(
                        "  [{}] {} {:?} \"{}\" -> {} ({}x{})",
                        i + 1,
                        elem.id,
...
///
/// This is the compatibility entrypoint used by wrapper binaries such as
/// `skyvm-guest-cli`.
...
// ============================================================================
// iOS-specific HID and touch handling
// ============================================================================
...
// ============================================================================
// Android-specific ADB command handling
// ============================================================================
