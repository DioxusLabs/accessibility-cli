...
repo="${ACCESSIBILITY_CLI_REPO:-DioxusLabs/accessibility-cli}"
...
usage() {
  cat <<'USAGE'
Usage: scripts/install-from-ci.sh [options]

Contributor-only helper. Downloads all unexpired GitHub Actions artifacts
from the selected workflow run so you can run pre-release builds without
waiting for a tagged release.

For end-user installs, use the published GitHub Releases (cargo-dist
generates a `curl ... | sh` installer URL on each release page) or
`cargo install accessibility-cli`.
...
Options:
  ...
  --repo owner/name       GitHub repository. Defaults to ACCESSIBILITY_CLI_REPO
                          or DioxusLabs/accessibility-cli.
