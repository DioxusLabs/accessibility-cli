//! Application connection and control for the SkyVM API.
...
impl App {
    ...
    /// ```
    pub async fn connect(pid: u32, platform: Platform) -> SkyVMResult<Self> {
    ...
    /// ```
    pub async fn focused() -> SkyVMResult<Self> {
    ...
    /// ```
    pub async fn with_config(config: AppConfig) -> SkyVMResult<Self> {
    ...
    async fn create_adapter(config: &AppConfig) -> SkyVMResult<TargetedAccessibility> {
        match config.platform {
            #[cfg(target_os = "macos")]
            Platform::MacOS => TargetedAccessibility::new_macos(config.pid).map_err(|e| {
                SkyVMError::ConnectionFailed {
                    message: format!("Failed to create macOS adapter: {}", e),
                }
            }),
            #[cfg(target_os = "macos")]
            Platform::IOSSimulator => TargetedAccessibility::new_ios(config.udid.as_deref())
                .map_err(|e| SkyVMError::ConnectionFailed {
    ...
    /// ```
    pub async fn wait_for_locator(&self, selector: &str) -> SkyVMResult<Element> {
    ...
    pub async fn refresh(&self) -> SkyVMResult<()> {
    ...
    pub async fn tree(&self) -> SkyVMResult<ElementTree> {
    ...
    pub async fn fresh_tree(&self) -> SkyVMResult<ElementTree> {
        let mut inner = self.inner.lock().await;
        inner.clear_cache();
        inner
            .get_tree(&TreeFilter::default())
    ...
    pub async fn screenshot(&self) -> SkyVMResult<Screenshot> {
        let inner = self.inner.lock().await;
        inner
            .capture_screen()
            .map_err(|e: anyhow::Error| SkyVMError::ScreenshotFailed {
                message: e.to_string(),
            })
    }

    /// Get the screen bounds for the application.
    pub async fn screen_bounds(&self) -> SkyVMResult<Rect> {
        let inner = self.inner.lock().await;
        inner
            .get_screen_bounds()
            .await
    ...
    pub async fn annotated_screenshot(
        ...
        selector: Option<&str>,
        labels: bool,
    ) -> SkyVMResult<AnnotatedScreenshot> {
    ...
    /// ```
    pub async fn keystroke(&self, key_spec: &str) -> SkyVMResult<()> {
        let mut inner = self.inner.lock().await;
        inner
    ...
    /// ```
    pub async fn type_text(&self, text: &str) -> SkyVMResult<()> {
        let mut inner = self.inner.lock().await;
        inner
            .type_raw(text)
            .await
