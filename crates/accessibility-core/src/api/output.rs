//! Output formatting utilities for the SkyVM API.
...
impl Default for LlmQueryPrinter {
    fn default() -> Self {
        Self {
            structure_only: false,
        }
...
fn is_structural_node(elem: &Element) -> bool {
    ...
    let has_label = elem.title.as_ref().map_or(false, |t| !t.is_empty())
        || elem.description.as_ref().map_or(false, |d| !d.is_empty());
...
fn has_printable_label(elem: &Element) -> bool {
    elem.title.as_ref().map_or(false, |t| !t.is_empty())
        || elem.description.as_ref().map_or(false, |d| !d.is_empty())
}
...
fn is_llm_relevant(elem: &Element) -> bool {
    let has_label = elem.title.as_ref().map_or(false, |t| !t.is_empty())
        || elem.description.as_ref().map_or(false, |d| !d.is_empty())
        || elem.help.as_ref().map_or(false, |h| !h.is_empty())
        || elem.identifier.as_ref().map_or(false, |i| !i.is_empty())
        || elem
            .value
            .as_ref()
            .map_or(false, |v| !v.is_empty() && v.len() > 1);
    ...
    if has_label && !elem.actions.is_empty() && elem.bounds.is_some() {
