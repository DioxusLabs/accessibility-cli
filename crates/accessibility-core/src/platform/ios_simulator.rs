...
// =============================================================================
// Indigo Protocol Structures (HID injection)
// =============================================================================
...
impl DispatcherState {
    fn new() -> Self {
        ...
        let queue_label = b"com.skyvm.accessibility_translator.callback\0";
...
// =============================================================================
// TranslationDispatcher - implements AXPTranslationTokenDelegateHelper
// =============================================================================
...
fn create_dispatcher_class() -> &'static AnyClass {
    ...
    DISPATCHER_CLASS.get_or_init(|| {
        let mut builder = ClassBuilder::new(c"SkyVMTranslationDispatcher", NSObject::class())
...
// =============================================================================
// SimulatorHID - HID injection via Indigo protocol
// =============================================================================
...
impl SimulatorHID {
    ...
    pub fn new(device: *mut AnyObject) -> Result<Self> {
        ...
        let msg_for_button: IndigoMessageForButtonFn = unsafe {
            let sym = libc::dlsym(
                handle,
                b"IndigoHIDMessageForButton\0".as_ptr() as *const c_char,
            );
            if sym.is_null() {
        ...
        let msg_for_touch: IndigoMessageForTouchFn = unsafe {
            let sym = libc::dlsym(
                handle,
                b"IndigoHIDMessageForMouseNSEvent\0".as_ptr() as *const c_char,
            );
            if sym.is_null() {
        ...
        let msg_for_keyboard: IndigoMessageForKeyboardFn = unsafe {
            let sym = libc::dlsym(
                handle,
                b"IndigoHIDMessageForKeyboardArbitrary\0".as_ptr() as *const c_char,
            );
            if sym.is_null() {
                return Err(anyhow!(
        ...
        let queue_label = b"com.skyvm.hid\0";
...
impl IOSSimulatorAccessibility {
    ...
    unsafe fn build_element_tree(
        ...
        let focused = false; // TODO: Check focus state
    ...
    pub fn capture_screen(&self) -> Result<Screenshot> {
        ...
        let screenshot_path = temp_dir.join(format!("skyvm_screenshot_{}.png", std::process::id()));
...
// =============================================================================
// AccessibilityReader Implementation for iOS Simulator
// =============================================================================
...
// =============================================================================
// IOSAdapter Implementation for iOS Simulator
// =============================================================================
